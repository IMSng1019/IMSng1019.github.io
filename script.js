document.addEventListener('DOMContentLoaded', function() {
    ...
    function autoSetThemeByTime() {
        ...
        // 定义白天时段（例如：6点到18点为白天，其他时间为夜晚）
        const isDayTime = currentHour >= 6 && currentHour < 18;
        
        // 检查是否有用户手动设置的主题（用户点击了主题切换按钮）
        const userSetTheme = localStorage.getItem('userSetTheme');
        
        // 如果用户手动设置了主题，则优先使用用户的设置
        if (userSetTheme) {
            if (userSetTheme === 'dark' && !document.body.classList.contains('dark-mode')) {
                enableDarkMode();
            } else if (userSetTheme === 'light' && document.body.classList.contains('dark-mode')) {
                enableLightMode();
            }
        } else {
            // 用户没有手动设置主题，根据时间自动切换
            if (isDayTime) {
                // 如果是白天且当前是暗色模式，则切换到亮色模式
                if (document.body.classList.contains('dark-mode')) {
                    enableLightMode();
        ...
        // 添加定时器，每分钟检查一次时间变化
        setTimeout(autoSetThemeByTime, 60000);
    }
    ...
    themeToggleBtn.addEventListener('click', () => {
        ...
        if (document.body.classList.contains('dark-mode')) {
            ...
            // 记录用户手动设置了主题
            localStorage.setItem('userSetTheme', 'dark');
            ...
            // 记录用户手动设置了主题
            localStorage.setItem('userSetTheme', 'light');
